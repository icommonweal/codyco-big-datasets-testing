...
% ad-hoc workaround for return point dataset that 
% use the timestamp of the console and not of the pc104
delta_time = 14612.5;
for i=1:n_datasets
    % check if dataset names finish with /return_point/data.log
    if( strcmp(varargin{i}(end-21:end),'/return_point/data.log') )
        raw_data_log{i}(:,2) = raw_data_log{i}(:,2)+delta_time;
    end
end
...
for i = 1:n_datasets
    ...
    raw_data_log{i} = raw_data_log{i}((1-initial_samples_dropped):(end-final_samples_dropped),:);
end
...
% We choose the easyest option to avoid adding unnecessary complexity.
